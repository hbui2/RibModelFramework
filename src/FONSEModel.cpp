...
#endif
	for (unsigned i = 0u; i < aaListing.size(); i++)
	{
		curAA = aaListing[i];
...
void FONSEModel::simulateGenome(Genome & genome)
{
	...
	for (unsigned geneIndex = 0; geneIndex < genome.getGenomeSize(); geneIndex++) //loop over all genes in the genome
	{
		...
		std::string codon = ct->indexToCodon((unsigned)Parameter::randUnif(61.0, 64.0)); //randomly choose a stop codon, from range 61-63
		tmpSeq += codon;
